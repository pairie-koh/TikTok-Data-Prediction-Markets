"""
Classify YouTube prediction market videos as INFORMATION vs TRADING.
...
Usage:
    ...
    python youtube/scripts/classify_info_vs_trading.py

    # Preview without API calls:
    python youtube/scripts/classify_info_vs_trading.py --dry-run
"""
...
def classify(client, platforms, title, description, transcript):
...
def main():
    ...
    for i, v in enumerate(remaining):
        ...
        try:
            result = classify(client, platforms, title, description, transcript)
            progress[vid] = result
            api_calls += 1
    ...
    for plat, cnts in sorted(platform_data.items()):
        ...
        print(f"  {plat:<20} INFO: {info:>4} ({info/total*100:.0f}%)  TRADING: {trade:>4} ({trade/total*100:.0f}%)  total: {total}")

    # Engagement comparison
    print(f"\nEngagement by category:")
    import statistics
    for cat in ["INFORMATION", "TRADING"]:
        cat_vids = [v for v in videos if progress.get(v.get("video_id", "")) == cat]
        if not cat_vids:
            continue
        views = [safe_int(v.get("views")) for v in cat_vids]
        likes = [safe_int(v.get("likes")) for v in cat_vids]
        print(f"  {cat}:")
